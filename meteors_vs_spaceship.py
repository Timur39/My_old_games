...
def show_go_screen():
    ...
    draw_text(screen, "Нажми любую кномку для новой игры", 22,
              W / 2, H / 2)
...
def win_screen():
    screen.blit(background, background_rect)
    draw_text(screen, "Ты побидил:)", 64, W / 2, H / 4)
    draw_text(screen, "Нажми любую кнопку для новой игры", 22,
              W / 2, H / 2)
...
meteor_images = pygame.image.load(path.join(img_folder, "meteorBrown_med1.png"))
...
# meteor big, mini, normal
meteor_mini_img = pygame.transform.scale(meteor_images, (25, 25))
meteor_mini_img.set_colorkey(BLACK)
meteor_big_img = pygame.transform.scale(meteor_images, (65, 65))
meteor_big_img.set_colorkey(BLACK)
meteor_list = [meteor_mini_img, meteor_images, meteor_big_img]
...
while running:
    ...
    # проверьте, не попала ли пуля в моба
